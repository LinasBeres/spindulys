...
public:
	CPULight() = default;
	CPULight(const AffineSpace3f& transform)
		: Light(transform)
	{ }
	...
	SampleDirection(const Interaction& it, const Vec2f& sample, uint32_t active) const = 0;

	virtual float
	PdfDirection(const Interaction& it, const DirectionSample& ds, uint32_t active) const = 0;
